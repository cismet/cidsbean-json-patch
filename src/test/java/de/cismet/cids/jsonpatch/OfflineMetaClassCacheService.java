...
public class OfflineMetaClassCacheService implements MetaClassCacheService {
    ...
    protected static final HashMap<Integer, MetaClass> allClassesById = new HashMap<Integer, MetaClass>();
    protected static final HashMap<String, MetaClass> allClassesByTableName = new HashMap<String, MetaClass>();
            ...
            while (scanner.hasNext()) {
                ...
                try {
                    final CidsClass cidsClass = MAPPER.readValue(
                        new BufferedReader(new InputStreamReader(classLoader.getResourceAsStream(jsonFile))),
                        CidsClass.class
                    );
                    LOGGER.debug(cidsClass.getKey() + " deserialized");
                    final MetaClass metaClass = CidsClassFactory
                        .getFactory()
